...
export async function generateDailyQuiz(profile?: any) {
  try {
    const ai = getAIInstance();
    const prompt = `Gere 3 perguntas de múltipla escolha sobre nutrição angolana, alimentos típicos e saúde.
    REGRAS:
    1. CONTEXTO: Focado em Angola (muamba, funge, paracuca, kizaca, múcua, etc).
    2. PERFIL: Considere que o usuário é ${profile?.profile_type || 'Adulto'} de ${profile?.age || ' idade variada'}.
    3. FORMATO: JSON estruturado.
      ...
      "questions": [
        {
          ...
          "correctIndex": 0,
          "explanation": "Breve explicação personalizada kidiaNutri"
        }
      ]
    }`;
